...
CompletedTasksFile = "completed_tasks.txt"
...
def taskComplete():
    ...
    try:
        ...
        if 0 <= complete < len(tasks):
            completed_task = tasks.pop(complete)
            with open(FileName, "w") as file:
                file.writelines(tasks)
            with open(CompletedTasksFile, "a") as completed_file:
                completed_file.write(completed_task)
            print(f"{completed_task.strip()} görevi tamamlandı olarak işaretlendi.")
        else:
            print("Geçersiz görev numarası.")
    except (ValueError, IndexError):
        print("Lütfen geçerli bir sayı giriniz.")
